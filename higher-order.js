...
//Code Here
let totalPopulation = populations.reduce((el, i) => {
  return el + i
})
...
let orderTotals = orders.map((el) => {
  let salestax = el.price * el.tax
  return el.price + salestax
})
...
// let bobsTotal = purches.reduce((acc, el)=> {
//   return acc + el.price
// }, 0)
const bobsTotal= purchases.reduce(function(acc, el, i, arr){
  return acc + el.price
}, }
